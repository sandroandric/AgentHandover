...
class FocusProcessor:
    ...
    def process_session(
        ...
        *,
        ...
        # Step 0a: Filter out OpenMimic's own events
        events = self._filter_own_events(events)
        if not events:
            return GeneratedSOP(
                sop={}, success=False, error="All events were self-referential (OpenMimic's own UI)"
            )
    ...
    _OWN_APP_PATTERNS = ("openmimic", "openmimicapp", "oc-apprentice", "oc_apprentice")
    _OWN_TITLE_PATTERNS = ("openmimic",)

    def _filter_own_events(self, events: list[dict]) -> list[dict]:
        """Remove events generated by OpenMimic's own UI.

        Filters events where the app name, bundle ID, or window title
        references OpenMimic or its sub-processes, so the SOP only
    ...
    def _is_own_event(self, event: dict) -> bool:
        """Check whether an event originates from OpenMimic's own UI."""
